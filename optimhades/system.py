...
class Planet (object):
    ...
    #assumed 1/2 day resolution
    def tickUpgrade(self):
        if(self.upgradeTime > 0):
            if(self.upgradeTime <= 12 * hour):
                print("UPGRADED!:%s %s"%(self.type,self.level+1))
                self.level += 1
                self.upgradeTime = 0
            else:
                self.upgradeTime -= 12 * hour
        
    def upgrade(self):
        if(self.level == 1):
            self.level = 2
        self.upgradeTime = PLANET_UPGRADE_TIMES[self.level]

    def isLevelMax(self):
        return self.level >= PLANET_MAX_LEVELS[self.tier]
    ...
    def typeUpCost(self):
        if(self.type == 'Station'):
            return STATION_UPGRADE_COSTS[self.level]
        else:
            return PLANET_UPGRADE_COSTS[self.level]
...
class IcePlanet (Planet):
    ...
    SHIPMENT_YIELD = {
        4: (
            ...
            1503, 1608, 1721, 1841, 1970,
            2108, 2243, 2394, 2555, 2726,
        ),
    }
...
class System (object):
    ...
    def creditCap(self):
        return sum([planet.creditStorage() for planet in self.planets])

    def halfDayCreditYield(self):
        return 12 * sum([planet.cargoPerHour() for planet in self.planets])    

    def tickUpgrades(self):
        [planet.tickUpgrade() for planet in self.planets]

    def findPlanetToUpgrade(self):
        minUpCost = None
        for planet in self.planets:
              if minUpCost == None:
                minUpCost = planet
              else:
                if planet.typeUpCost() < minUpCost.typeUpCost():
                      if not planet.isLevelMax() and planet.upgradeTime == 0:
                        minUpCost = planet
        return minUpCost
